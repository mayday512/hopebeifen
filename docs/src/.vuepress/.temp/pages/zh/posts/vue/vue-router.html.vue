...
<p>在 Web 开发过程中，经常遇到路由的概念。那么到底什么是路由呢？简单来说，路由就是 <strong>URL 到函数的映射</strong>。</p>
<p>路由这个概念本来是由后端提出来的，在以前用模板引擎开发页面的时候，是使用路由返回不同的页面，大致流程是这样的：</p>
<ol>
<li>浏览器发出请求；</li>
<li>服务器监听到 80 或者 443 端口有请求过来，并解析 UR L路径；</li>
<li>服务端根据路由设置，查询相应的资源，可能是 html 文件，也可能是图片资源......，然后将这些资源处理并返回给浏览器；</li>
<li>浏览器接收到数据，通过content-type决定如何解析数据
简单来说，路由就是用来跟后端服务器交互的一种方式，通过不同的路径来请求不同的资源，请求HTML页面只是路由的其中一项功能。</li>
</ol>
<h2 id="服务器端路由" tabindex="-1"><a class="header-anchor" href="#服务器端路由" aria-hidden="true">#</a> 服务器端路由</h2>
<p>当服务端接收到客户端发来的 HTTP 请求时，会根据请求的 URL，找到相应的映射函数，然后执行该函数，并将函数的返回值发送给客户端。</p>
<p>对于最简单的静态资源服务器，可以认为，所有 URL 的映射函数就是一个文件读取操作。对于动态资源，映射函数可能是一个数据库读取操作，也可能进行一些数据处理，等等。</p>
<h2 id="客户端路由" tabindex="-1"><a class="header-anchor" href="#客户端路由" aria-hidden="true">#</a> 客户端路由</h2>
<p>服务端路由会造成服务器压力比较大，而且用户访问速度也比较慢。在这种情况下，出现了单页应用。</p>
<p>单页应用，就是只有一个页面，用户访问网址，服务器返回的页面始终只有一个，不管用户改变了浏览器地址栏的内容或者在页面发生了跳转，服务器不会重新返回新的页面，而是通过相应的js操作来实现页面的更改。
前端路由其实就是：通过地址栏内容的改变，显示不同的页面。</p>
<p>前端路由的优点：</p>
<p>前端路由可以让前端自己维护路由与页面展示的逻辑，每次页面改动不需要通知服务端。</p>
<p>更好的交互体验：不用每次从服务端拉取资源。</p>
<p>前端路由的缺点：</p>
<p>使用浏览器的前进、后退键时会重新发送请求，来获取数据，没有合理利用缓存。</p>
<p>前端路由实现原理：
本质就是监测 URL 的变化，通过拦截 URL 然后解析匹配路由规则。</p>
<h2 id="前端路由实现方式" tabindex="-1"><a class="header-anchor" href="#前端路由实现方式" aria-hidden="true">#</a> 前端路由实现方式</h2>
<ol>
<li>
<p>hash模式（location.hash + hashchange 事件）
hash 模式的实现方式就是通过监听 URL 中的 hash 部分的变化，触发haschange事件，页面做出不同的响应。但是 hash 模式下，URL 中会带有 #，不太美观。</p>
</li>
<li>
<p>history模式
history 路由模式的实现，基于 HTML5 提供的 History 全局对象，它的方法有：</p>
</li>
<li>
<p>history.go()：在会话历史中向前或者向后移动指定页数</p>
</li>
<li>
<p>history.forward()：在会话历史中向前移动一页，跟浏览器的前进按钮功能相同</p>
</li>
<li>
<p>history.back()：在会话历史记录中向后移动一页，跟浏览器的后腿按钮功能相同</p>
</li>
<li>
<p>history.pushState()：向当前浏览器会话的历史堆栈中添加一个状态，会改变当前页面url，但是不会伴随这刷新</p>
</li>
<li>
<p>history.replaceState()：将当前的会话页面的url替换成指定的数据，replaceState 会改变当前页面的url，但也不会刷新页面</p>
</li>
<li>
<p>window.onpopstate：当前活动历史记录条目更改时，将触发popstate事件
history路由的实现，主要是依靠pushState、replaceState和window.onpopstate实现的。但是有几点要注意：</p>
</li>
<li>
<p>当活动历史记录条目更改时，将触发 popstate 事件；</p>
</li>
<li>
<p>调用history.pushState()或history.replaceState()不会触发 popstate 事件
popstate 事件只会在浏览器某些行为下触发，比如：点击后退、前进按钮（或者在 JavaScript 中调用history.back()、history.forward()、history.go()方法）</p>
</li>
<li>
<p>a 标签的锚点也会触发该事件</p>
</li>
</ol>
<h2 id="对-pushstate-和-replacestate-行为的监听" tabindex="-1"><a class="header-anchor" href="#对-pushstate-和-replacestate-行为的监听" aria-hidden="true">#</a> 对 pushState 和 replaceState 行为的监听</h2>
<p>如果想监听 pushState 和 replaceState 行为，可以通过在方法里面主动去触发 popstate 事件，另一种是重写history.pushState，通过创建自己的eventedPushState自定义事件，并手动派发，实际使用过程中就可以监听了。具体做法如下：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">eventedPushState</span><span class="token punctuation">(</span><span class="token parameter">state<span class="token punctuation">,</span> title<span class="token punctuation">,</span> url</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="router-和-route-的区别" tabindex="-1"><a class="header-anchor" href="#router-和-route-的区别" aria-hidden="true">#</a> router 和 route 的区别</h2>
<p>route 就是一条路由，它将一个 URL 路径和一个函数进行映射。而 router 可以理解为一个容器，或者说一种机制，它管理了一组 route。</p>
<p>概括为：route 只是进行了 URL 和函数的映射，在当接收到一个 URL 后，需要去路由映射表中查找相应的函数，这个过程是由 router 来处理的。</p>
<h3 id="动态路由和静态路由" tabindex="-1"><a class="header-anchor" href="#动态路由和静态路由" aria-hidden="true">#</a> 动态路由和静态路由</h3>
<p>静态路由</p>
<p>静态路由只支持基于地址的全匹配。</p>
<p>动态路由</p>
<p>动态路由除了可以兼容全匹配外还支持多种”高级匹配模式“，它的路径地址中含有路径参数，使得它可以按照给定的匹配模式将符合条件的一个或多个地址映射到同一个组件上。</p>
<p>动态路由一般结合角色权限控制使用。</p>
<p>动态路由的存储有两种方式：</p>
<ol>
<li>
<p>将路由存储到前端</p>
...
<h2 id="实现一个路由" tabindex="-1"><a class="header-anchor" href="#实现一个路由" aria-hidden="true">#</a> 实现一个路由</h2>
<p>一个简单的Router应该具备哪些功能
以 Vue为例，需要有<code v-pre> &lt;router-link&gt;</code>链接、<code v-pre>&lt;router-view&gt;</code>容器、component组件和path路由路径：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> routes <span class="token operator">=</span> <span class="token punctuation">[</span><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span><span class="token punctuation">]</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>综上，一个简单的 Router 应该具备以下功能：</p>
<ol>
<li>容器（组件）</li>
<li>路由</li>
<li>业务组件 &amp; 链接组件</li>
</ol>
<h2 id="不借助第三方工具库-如何实现路由" tabindex="-1"><a class="header-anchor" href="#不借助第三方工具库-如何实现路由" aria-hidden="true">#</a> 不借助第三方工具库，如何实现路由</h2>
<p>不借助第三方工具库实现路由，我们需要思考以下几个问题：</p>
<ol>
<li>如何实现自定义标签，如vue的<code v-pre>&lt;router-view&gt;</code>，React的<code v-pre>&lt;Router&gt;</code></li>
<li>如何实现业务组件</li>
<li>如何动态切换路由</li>
</ol>
<h3 id="根据对前端路由-history-模式的理解-将大致过程用如下流程图表示" tabindex="-1"><a class="header-anchor" href="#根据对前端路由-history-模式的理解-将大致过程用如下流程图表示" aria-hidden="true">#</a> 根据对前端路由 history 模式的理解，将大致过程用如下流程图表示：</h3>
<p><a href="https://postimg.cc/GHWs14M3" target="_blank" rel="noopener noreferrer"><img src="https://i.postimg.cc/L8XkLfLP/640.jpg" alt="640.jpg" loading="lazy"><ExternalLinkIcon/></a></p>
<h3 id="如果不借助第三方库-我们选择使用-web-components-。web-components由三项主要技术组成-它们可以一起使用来创建封装功能的定制元素。" tabindex="-1"><a class="header-anchor" href="#如果不借助第三方库-我们选择使用-web-components-。web-components由三项主要技术组成-它们可以一起使用来创建封装功能的定制元素。" aria-hidden="true">#</a> 如果不借助第三方库，我们选择使用 Web components 。Web Components由三项主要技术组成，它们可以一起使用来创建封装功能的定制元素。</h3>
<p>Custom elements（自定义元素） ：一组JavaScript API，允许我们定义 custom elements及其行为，然后可以在界面按照需要使用它们。</p>
<p>Shadow DOM（影子DOM） ：一组JavaScript API，用于将封装的“影子”DOM树附加到元素（与主文档分开呈现）并控制关联的功能。通过这种方式，可以保持元素的功能私有。</p>
<p>HTML template（HTML模版） ：<code v-pre>&lt;template&gt;</code>和<code v-pre>&lt;slot&gt;</code>可以编写不在页面显示的标记模板，然后它们可以作为自定义元素结构的基础被多次重用。
另外还需要注意 Web Components 的生命周期：</p>
<p>connectedCallback：当 custom element 首次被插入文档DOM时，被调用</p>
<p>disconnectedCallback：当 custom element 从文档DOM中删除时，被调用</p>
<p>adoptedCallback：当custom element 被移动到新的文档时，被调用</p>
...
<p>Shadow DOM 特有的术语：</p>
<ul>
<li>Shadow host：一个常规DOM节点，Shadow DOM 会被附加到这个节点上</li>
<li>Shadow tree：Shadow DOM 内部的 DOM 树</li>
<li>Shadow boundary：Shadow DOM 结束的地方，也是常规DOM开始的地方</li>
<li>Shadow root：Shadow tree 的根节点</li>
</ul>
<p>Shadow DOM的重要参数mode：</p>
<ul>
<li>open：shadow root 元素可以从 js 外部访问根节点</li>
<li>close ：拒绝从 js 外部访问关闭的 shadow root 节点</li>
<li>语法：<code v-pre>const shadow = this.attachShadow({mode:closed});</code></li>
</ul>
<h3 id="通过自定义标签创建容器组件、路由、业务组件和链接组件标签-使用customelementregistry-define-注册自定义元素。其中-custom-elements-的简单写法举例" tabindex="-1"><a class="header-anchor" href="#通过自定义标签创建容器组件、路由、业务组件和链接组件标签-使用customelementregistry-define-注册自定义元素。其中-custom-elements-的简单写法举例" aria-hidden="true">#</a> 通过自定义标签创建容器组件、路由、业务组件和链接组件标签，使用CustomElementRegistry.define()注册自定义元素。其中，Custom elements 的简单写法举例：</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="组件的实现可以使用-web-components-但是这样有缺点-我们没有打包引擎处理-web-components组件-将其全部加载过来。" tabindex="-1"><a class="header-anchor" href="#组件的实现可以使用-web-components-但是这样有缺点-我们没有打包引擎处理-web-components组件-将其全部加载过来。" aria-hidden="true">#</a> 组件的实现可以使用 Web Components，但是这样有缺点，我们没有打包引擎处理 Web Components组件，将其全部加载过来。</h3>
<p>为了解决以上问题，我们选择动态加载，远程去加载一个 html 文件。html文件里面的结构如下：支持模版(template)，脚本(template)，脚本(script)，样式(style)，非常地像vue。组件开发模版如下：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="监听路由的变化" tabindex="-1"><a class="header-anchor" href="#监听路由的变化" aria-hidden="true">#</a> 监听路由的变化：</h3>
<p>popstate可以监听大部分路由变化的场景，除了pushState 和 replaceState。</p>
<p>pushState 和 replaceState可以改变路由，改变历史记录，但是不能触发popstate事件，需要自定义事件并手动触发自定义事件，做出响应。</p>
<h3 id="总体架构" tabindex="-1"><a class="header-anchor" href="#总体架构" aria-hidden="true">#</a> 总体架构</h3>
<p><a href="https://postimg.cc/gL6kMDbV" target="_blank" rel="noopener noreferrer"><img src="https://i.postimg.cc/Kj0k8q9H/640-1.jpg" alt="640-1.jpg" loading="lazy"><ExternalLinkIcon/></a></p>
<h3 id="组件功能拆解分析如下" tabindex="-1"><a class="header-anchor" href="#组件功能拆解分析如下" aria-hidden="true">#</a> 组件功能拆解分析如下：</h3>
<p>链接组件 — CustomLink(c-link)</p>
<p>当用户点击<code v-pre>&lt;c-link&gt;</code>标签后，通过event.preventDefault();阻止页面默认跳转。根据当前标签的to属性获取路由，通过history.pushState(&quot;&quot;,&quot;&quot;,to)进行路由切换。</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">//  &lt;c-link to="/" class="c-link">首页&lt;/c-link></span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>容器组件 — CustomRouter(c-router)</p>
<p>主要是收集路由信息，监听路由信息的变化，然后加载对应的组件</p>
<p>路由 — CustomRoute(c-route)</p>
<p>主要是提供配置信息，对外提供getData 的方法</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 优先于c-router注册</span>
...
<span class="token operator">&lt;</span>style<span class="token operator">></span>
    ...
    <span class="token punctuation">}</span>
<span class="token operator">&lt;</span><span class="token operator">/</span>style<span class="token operator">></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>about.html</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token operator">&lt;</span>template<span class="token operator">></span>
    About Me<span class="token operator">!</span>
<span class="token operator">&lt;</span><span class="token operator">/</span>template<span class="token operator">></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>route.js</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> oriPushState <span class="token operator">=</span> history<span class="token punctuation">.</span>pushState<span class="token punctuation">;</span>
