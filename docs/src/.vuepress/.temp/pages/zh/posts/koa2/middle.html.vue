<template><div><h1 id="洋葱模型" tabindex="-1"><a class="header-anchor" href="#洋葱模型" aria-hidden="true">#</a> 洋葱模型</h1>
<h2 id="中间件" tabindex="-1"><a class="header-anchor" href="#中间件" aria-hidden="true">#</a> 中间件</h2>
<p>中间件的作用就是，在你上下两个事件通讯的过程中强行塞入一个事件，这个事件可以起到承上启下的作用，并在运行应用的时候做一些只关于部分模块化功能。
<img src="https://upload-images.jianshu.io/upload_images/14243841-4f932a4d2ce4391c.png?imageMogr2/auto-orient/strip|imageView2/2/w/478/format/webp" alt="" loading="lazy"></p>
<p><strong>定义一个中间件就是定义一个函数</strong></p>
<p><strong>ctx 上下文 next 下一个中间件函数本质是一个promise 可以层层调用</strong></p>
<p><strong>通常对资源的操作是异步的，读文件 发送http（axios） 操作数据库（squelize）</strong></p>
<p><strong>中间件可以return 由上一个中间件next接收</strong></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">//commonJS 导入</span>
...
<p>异步处理的终极方案</p>
<p><strong>async await保证一定按照洋葱模型执行 async：强制包装成为promise await:表达式取值 promise取值 阻塞当前线程(异步变同步)</strong></p>
<p><strong>通常对资源的操作是异步的，读文件 发送http（axios） 操作数据库（squelize）</strong></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">//commonJS 导入</span>
<span class="token keyword">const</span> Koa <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'koa'</span><span class="token punctuation">)</span>
<span class="token comment">//实例化应用程序对象</span>
